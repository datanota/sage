...
# ############################################################## solutions section
    ...
    def get_solutions_widgets(self):
        ...
        for solution in list(self.dict_solutions().keys()):
            btn = self.solutions_home_buttons(
                text=f"{solution}: {self.dict_solutions().get(solution)().get('model_type')}",
                sage_theme_dict=self.dict_sage_theme_colors()
            )
            btn.bind(on_press=lambda x: self.callback_top_nav('solution_generic_screen'))
            self.root.ids.solutions_home_layout.add_widget(btn)
            solution_data = self.dict_solutions().get(solution)().get('data')(
                data_file=f'{self.generated_data_path}{solution.lower()}.xlsx'
            )
            btn.bind(on_press=partial(self.solution_layout, solution, solution_data))

    def solution_layout(self, *args):
        solution = args[0]
        solution_data = args[1]
        try:
            solution_layout = self.dict_solutions().get(solution)().get('layout')(
                tp=self.get_generic_tab_panel(total_tabs=3),
                sage_theme_dict=self.dict_sage_theme_colors(),
                assets_path=self.path_to_assets,
                data=solution_data
            )
            self.root.ids.solution_generic_layout.add_widget(solution_layout)
        except Exception as e:
            print(f'there is an issue while populating solution layout: {e}')
